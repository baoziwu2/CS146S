...
from server.models import MessageFormat, SearchResultItem
            ...
            try:
                ...
                for msg in messages:
                    ...
                    if msg_id and msg_id not in seen_ids:
                        all_message_ids.append({"id": msg_id, "threadId": msg.get("threadId", "")})
                        seen_ids.add(msg_id)
                ...
                if status_code == 401:
                    ...
                    return {"error": "api_error", "message": f"Gmail API error: {e}", "results": []}
        ...
        for i, msg_info in enumerate(all_message_ids):
            ...
            if i < enrich_count:
                try:
                    ...
                    results.append(SearchResultItem(id=msg_id, thread_id=thread_id))
            else:
    ...
    def gmail_get_message(self, *, message_id: str, fmt: MessageFormat = "full") -> dict[str, Any]:
        ...
        try:
            ...
            if status_code == 404 or (status_code == 400 and "Invalid id" in str(e)):
                ...
                return {"error": "api_error", "message": f"Gmail API error: {e}"}
        except Exception as e:
            return {"error": "unknown_error", "message": f"Unexpected error: {e}"}
